...
The column 1 and 2 represent that the order of (Nth) restriction fragment in given chromosomes. If multiple chromosomes data are used, the fragment number of the next chromosome is continuing from the last fragment number of the preceding chromosome. **The fragment number must be perfectly matched with the 'unit.bed' file that can be generated by ```make1f_bed.py```**. You can easily get the matrix file from a '.hic' file using ```utils/dumpMatrix.py``` script. 
**Chromosomes used in all HiCORE scripts including utility scripts must be identical**
